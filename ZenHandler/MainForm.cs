...
namespace ZenHandler  //ApsMotionControl
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            {
                fileName = string.Format(@"{0}\Fw_Handler_IoMap_v1.6.xlsx", Application.StartupPath);
            }
            else if (Program.PG_SELECT == HANDLER_PG.AOI)
            {
                fileName = string.Format(@"{0}\Aoi_Handler_IoMap_v1.6.xlsx", Application.StartupPath);
            }
            else
            {
                //eeprom
                fileName = string.Format(@"{0}\EEprom_Handler_IoMap_v1.6.xlsx", Application.StartupPath);
            }
