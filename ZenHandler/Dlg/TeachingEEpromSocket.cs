...
{
    public partial class TeachingEEpromSocket : UserControl
    {
        ...
        //
        public TeachingEEpromSocket()
        {
            InitializeComponent();
            _syncContext = SynchronizationContext.Current;
            cts = new CancellationTokenSource();

            int[] inGridWid = new int[] { 130, 100, 100 };         //Grid Width

            myTeachingGrid = new Controls.TeachingGridView( Globalo.motionManager.socketEEpromMachine.MotorAxes, Globalo.motionManager.socketEEpromMachine.teachingConfig, inGridWid);
            ...
            //TeachResolution(Globalo.motionManager.socketEEpromMachine.teachingConfig.Resolution[SelectAxisIndex].ToString("0.#"));
        }
        ...
        {
            int i = 0;
            TeachBtnArr = new Button[] { BTN_TEACH_EEPROM_SOCKET_FRONT_X, BTN_TEACH_EEPROM_SOCKET_BACK_X };

            for (i = 0; i < TeachBtnArr.Length; i++)
            {
                TeachBtnArr[i].Text = Globalo.motionManager.socketEEpromMachine.MotorAxes[i].Name;
        ...
        private void changeBtnMotorNo(int MotorNo)
        {
            ...
            if (Globalo.motionManager.socketEEpromMachine.teachingConfig.Resolution.Count < 1)
            {
                Console.WriteLine("socketEEpromMachine Resolution Zero");
                return;
            }
            TeachResolution(Globalo.motionManager.socketEEpromMachine.teachingConfig.Resolution[MotorNo].ToString("0.#"));


        }
        public void MotorJogStop()
        {
            Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].Stop();
        }
        public async Task<bool> MotorJogMove(int nDic, double dSpeed)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    isSuccess = Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].JogMove(nDic, dSpeed);
        ...
        public async Task<bool> MotorRelMove(double dPos)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    isSuccess = Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].MoveAxis(dPos, AXT_MOTION_ABSREL.POS_REL_MODE,  false);
        ...
        {
            Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].ServoOn();
        }
        ...
        private void BTN_TEACH_SERVO_OFF_Click(object sender, EventArgs e)
        {
            ...
            Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].ServoOff();
        }

        private void BTN_TEACH_SERVO_RESET_Click(object sender, EventArgs e)
        {
            Globalo.motionManager.socketEEpromMachine.MotorAxes[SelectAxisIndex].AmpFaultReset();
        }
        ...
        private void BTN_TEACH_DATA_SAVE_Click(object sender, EventArgs e)
        {
            ...
            {
                Globalo.motionManager.socketEEpromMachine.teachingConfig = myTeachingGrid.GetTeachData(Globalo.motionManager.socketEEpromMachine.teachingConfig);
                
                
                double dResol = double.Parse(LABEL_TEACH_ROSOLUTION_VALUE.Text);
                Globalo.motionManager.socketEEpromMachine.teachingConfig.Resolution[SelectAxisIndex] = dResol;
                
                //Motor Speed 적용
                int length = Globalo.motionManager.socketEEpromMachine.MotorAxes.Length;

                for (int i = 0; i < length; i++)
                {
                    Globalo.motionManager.socketEEpromMachine.MotorAxes[i].Velocity = Globalo.motionManager.socketEEpromMachine.teachingConfig.Speed[i];
                    Globalo.motionManager.socketEEpromMachine.MotorAxes[i].Acceleration = Globalo.motionManager.socketEEpromMachine.teachingConfig.Accel[i];
